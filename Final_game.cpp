...
const char* Color[2]={"red","blue"};//红蓝总部名称
const char* Character[5]={"dragon","ninja","iceman","lion","wolf"};//士兵名称
const char* Weapon[3]={"sword","bomb","arrow"};//武器名称
...
/* arrow武器类 注意：两个相邻的武士可能同时放箭把对方射死。 */
class arrow
{
private:
    int used_times;//使用次数
...
public:
    ...
    {
        if (morale>0.8)
            printf("%.3d:40 %s dragon %d yelled in city %d\n",now_time,Color[color],id,_id);//dragon 在一次在它主动进攻的战斗结束后，如果还没有战死，而且士气值大于0.8，就会欢呼。
    }
...
{
    cout<<now_time<<":00 "/*这个地方有问题*/Color[_color]<<%s %d born\n",now_time,Color[_color],Character[_type],_id);//在每个整点，即每个小时的第0分， 双方的司令部中各有一个武士降生。
